...
import {
  ...
  fetchLineRouting,
  saveLineRouting,
  type LineAccountsResponse,
  type LineRoutingResponse,
} from '../../lib/adminApi';
import type { LineAccount, LineAccountPurpose, LineAccountIndustry, LineRouting } from '@/src/types/settings';
...
export default function AdminSettingsClient() {
  ...
  // --- LINE Accounts (Multi) ---
  const [lineAccounts, setLineAccounts] = useState<(LineAccount & { synthesized?: boolean })[]>([]);
  const [lineAccountsSynthesized, setLineAccountsSynthesized] = useState(false);
  const [lineAccountsLoading, setLineAccountsLoading] = useState(false);
  const [lineAccountsError, setLineAccountsError] = useState<string | null>(null);
  const [lineRouting, setLineRouting] = useState<LineRouting>({});
  const [lineAccountModalOpen, setLineAccountModalOpen] = useState(false);
  const [lineAccountEditing, setLineAccountEditing] = useState<(LineAccount & { synthesized?: boolean }) | null>(null);
  const [lineAccountForm, setLineAccountForm] = useState({
    name: '', key: '', purpose: 'booking' as LineAccountPurpose, industry: 'shared' as LineAccountIndustry,
    channelId: '', channelSecret: '', channelAccessToken: '', basicId: '', inviteUrl: '',
  });
  const [lineAccountSaving, setLineAccountSaving] = useState(false);
  ...
  // ============================================================
  // LINE Accounts (Multi) fetch / handlers
  // ============================================================
  const fetchLineAccountsState = async () => {
    setLineAccountsLoading(true);
    setLineAccountsError(null);
    try {
      const [acctRes, routingRes] = await Promise.all([
        fetchLineAccounts(tenantId),
        fetchLineRouting(tenantId),
      ]);
      setLineAccounts(acctRes.accounts || []);
      setLineAccountsSynthesized(acctRes.synthesized);
      setLineRouting(routingRes.routing || {});
    } catch (e: any) {
      setLineAccountsError(e?.message || 'LINEアカウント取得に失敗しました');
    } finally {
      setLineAccountsLoading(false);
    }
  };

  const openLineAccountModal = (account?: LineAccount & { synthesized?: boolean }) => {
    if (account && !account.synthesized) {
      setLineAccountEditing(account);
      setLineAccountForm({
        name: account.name, key: account.key, purpose: account.purpose, industry: account.industry,
        channelId: account.channelId, channelSecret: account.channelSecret, channelAccessToken: account.channelAccessToken,
        basicId: account.basicId || '', inviteUrl: account.inviteUrl || '',
      });
    } else {
      setLineAccountEditing(null);
      setLineAccountForm({
        name: '', key: '', purpose: 'booking', industry: 'shared',
        channelId: '', channelSecret: '', channelAccessToken: '', basicId: '', inviteUrl: '',
      });
    }
    setLineAccountModalOpen(true);
  };

  const handleLineAccountSave = async () => {
    if (!lineAccountForm.name || !lineAccountForm.channelAccessToken || !lineAccountForm.channelSecret) {
      setLineAccountsError('名前、アクセストークン、チャネルシークレットは必須です');
      return;
    }
    setLineAccountSaving(true);
    setLineAccountsError(null);
    try {
      if (lineAccountEditing) {
        await updateLineAccount(lineAccountEditing.id, lineAccountForm, tenantId);
      } else {
        await createLineAccount(lineAccountForm, tenantId);
      }
      setLineAccountModalOpen(false);
      await fetchLineAccountsState();
      showToast(lineAccountEditing ? 'アカウントを更新しました' : 'アカウントを追加しました', 'success');
    } catch (e: any) {
      setLineAccountsError(e?.message || '保存に失敗しました');
    } finally {
      setLineAccountSaving(false);
    }
  };

  const handleLineAccountDelete = async (id: string) => {
    if (!confirm('このLINEアカウントを無効化しますか？')) return;
    setLineAccountsLoading(true);
    try {
      await deleteLineAccount(id, tenantId);
      await fetchLineAccountsState();
      showToast('アカウントを無効化しました', 'success');
    } catch (e: any) {
      setLineAccountsError(e?.message || '削除に失敗しました');
    } finally {
      setLineAccountsLoading(false);
    }
  };
  ...
  const handleRoutingChange = async (patch: Partial<LineRouting>) => {
    ...
    try {
      ...
      await fetchLineAccountsState();
      showToast('ルーティングを更新しました', 'success');
    } catch (e: any) {
      setLineAccountsError(e?.message || 'ルーティング保存に失敗しました');
    } finally {
      setLineAccountsLoading(false);
    }
  };
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div className="flex-1">
              <h2 className="text-base font-semibold text-gray-900">LINEアカウント管理</h2>
              <p className="text-xs text-gray-500">複数のLINE公式アカウントを用途別に管理します</p>
            </div>
            <button
              onClick={() => openLineAccountModal()}
              disabled={lineAccountsLoading}
          ...
          {lineAccountsError && (
            <div className="flex items-start gap-1.5 mb-4 p-2 bg-red-50 border border-red-200 rounded text-xs text-red-700">
              <AlertCircle className="w-3.5 h-3.5 shrink-0 mt-0.5" />
              <div>{lineAccountsError}</div>
            </div>
          )}

          {lineAccountsSynthesized && lineAccounts.length > 0 && (
            <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg text-xs text-blue-700">
              既存のMessaging API設定が検出されました。「追加」から正式にアカウントを登録すると、マルチアカウント管理が有効になります。
            </div>
          )}

          {lineAccountsLoading ? (
            <div className="text-xs text-gray-500 py-4 text-center">読み込み中...</div>
          ) : lineAccounts.length === 0 ? (
            <div className="text-xs text-gray-400 py-4 text-center">LINEアカウントが登録されていません</div>
          ) : (
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-gray-200 text-left text-gray-500">
                    ...
                    <th className="pb-2 pr-3 font-medium">用途</th>
                    <th className="pb-2 pr-3 font-medium">業種</th>
                    <th className="pb-2 pr-3 font-medium">ステータス</th>
                    <th className="pb-2 pr-3 font-medium">Basic ID</th>
                    <th className="pb-2 font-medium">操作</th>
                  </tr>
                </thead>
                <tbody>
                  {lineAccounts.map((acct) => (
                    <tr key={acct.id} className="border-b border-gray-100 hover:bg-gray-50">
                      <td className="py-2 pr-3 font-medium text-gray-900">{acct.name}</td>
                      <td className="py-2 pr-3 text-gray-600">{acct.purpose}</td>
                      <td className="py-2 pr-3 text-gray-600">{acct.industry}</td>
                      <td className="py-2 pr-3">
                        <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-medium ${
                          acct.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'
                        }`}>
                          <span className={`w-1.5 h-1.5 rounded-full ${acct.status === 'active' ? 'bg-green-500' : 'bg-gray-400'}`} />
                          {acct.status === 'active' ? '有効' : '無効'}
                        </span>
                      </td>
                      <td className="py-2 pr-3 text-gray-500 font-mono">{acct.basicId || '-'}</td>
                      <td className="py-2">
                        <div className="flex gap-1">
                          {!acct.synthesized && (
                            <>
                              <button onClick={() => openLineAccountModal(acct)} className="px-2 py-1 text-indigo-600 hover:bg-indigo-50 rounded text-[10px]">編集</button>
                              {acct.status === 'active' && (
                                <button onClick={() => handleLineAccountDelete(acct.id)} className="px-2 py-1 text-red-600 hover:bg-red-50 rounded text-[10px]">無効化</button>
                              )}
                            </>
                          )}
                          {acct.synthesized && (
                            <span className="text-gray-400 text-[10px]">移行前</span>
                          )}
                        </div>
                      </td>
                    </tr>
          ...
          {/* ルーティング */}
          {lineAccounts.filter(a => a.status === 'active' && !a.synthesized).length > 0 && (
            <div className="mt-4 pt-4 border-t border-gray-200 space-y-4">
              {/* 予約用デフォルト */}
              {lineAccounts.filter(a => a.purpose === 'booking' && a.status === 'active' && !a.synthesized).length > 0 && (
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">予約用デフォルトアカウント</label>
                  <select
                    value={lineRouting.booking?.default || ''}
                    onChange={e => handleRoutingChange({ booking: { default: e.target.value } })}
                    className="w-full sm:w-64 px-2 py-1.5 text-xs border border-gray-300 rounded focus:ring-1 focus:ring-indigo-500 outline-none"
                  >
                    <option value="">未選択</option>
                    {lineAccounts.filter(a => a.purpose === 'booking' && a.status === 'active' && !a.synthesized).map(a => (
                      <option key={a.id} value={a.id}>{a.name}</option>
                    ))}
                  </select>
                  <p className="text-[10px] text-gray-400 mt-1">選択したアカウントの認証情報が予約・Webhook・AI・リマインド等に使用されます</p>
                </div>
              )}
              {/* 営業用（業種別） */}
              {lineAccounts.filter(a => a.purpose === 'sales' && a.status === 'active' && !a.synthesized).length > 0 && (
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-2">営業用アカウント（業種別 — LP CTA に使用）</label>
                  <div className="space-y-2">
                    {(['eyebrow', 'hair', 'nail', 'esthetic', 'dental'] as const).map(ind => {
                      const candidates = lineAccounts.filter(a => a.purpose === 'sales' && a.status === 'active' && !a.synthesized);
                      if (candidates.length === 0) return null;
                      const salesMap = (lineRouting.sales || {}) as Record<string, string>;
                      return (
                        <div key={ind} className="flex items-center gap-2">
                          <span className="w-16 text-xs text-gray-500">{ind}</span>
                          <select
                            value={salesMap[ind] || ''}
                            onChange={e => handleRoutingChange({ sales: { ...salesMap, [ind]: e.target.value } })}
                            className="flex-1 sm:w-48 px-2 py-1 text-xs border border-gray-300 rounded focus:ring-1 focus:ring-indigo-500 outline-none"
                          >
                            <option value="">未設定（env fallback）</option>
                            {candidates.map(a => (
                              <option key={a.id} value={a.id}>{a.name}{a.industry !== 'shared' ? ` (${a.industry})` : ''}</option>
                            ))}
                          </select>
                        </div>
                      );
                  ...
                  <p className="text-[10px] text-gray-400 mt-1">LP の「LINEで無料診断する」CTA のリンク先に使用されます。inviteUrl が必要です。</p>
                </div>
              )}
            </div>
          )}
        ...
        {/* LINE Account Modal */}
        {lineAccountModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 p-6 max-h-[90vh] overflow-y-auto">
              <h3 className="text-sm font-semibold text-gray-900 mb-4">
                {lineAccountEditing ? 'LINEアカウント編集' : 'LINEアカウント追加'}
              </h3>
              <div className="space-y-3">
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">表示名 *</label>
                  <input value={lineAccountForm.name} onChange={e => setLineAccountForm({ ...lineAccountForm, name: e.target.value })}
                    className="w-full px-2 py-1.5 text-xs border border-gray-300 rounded focus:ring-1 focus:ring-indigo-500 outline-none" placeholder="例: 予約メイン" />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-medium text-gray-700 mb-1">用途</label>
                    <select value={lineAccountForm.purpose} onChange={e => setLineAccountForm({ ...lineAccountForm, purpose: e.target.value as LineAccountPurpose })}
                      className="w-full px-2 py-1.5 text-xs border border-gray-300 rounded focus:ring-1 focus:ring-indigo-500 outline-none">
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-700 mb-1">業種</label>
                    <select value={lineAccountForm.industry} onChange={e => setLineAccountForm({ ...lineAccountForm, industry: e.target.value as LineAccountIndustry })}
                      className="w-full px-2 py-1.5 text-xs border border-gray-300 rounded focus:ring-1 focus:ring-indigo-500 outline-none">
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">スラッグキー</label>
                  <input value={lineAccountForm.key} onChange={e => setLineAccountForm({ ...lineAccountForm, key: e.target.value })}
                    className="w-full px-2 py-1.5 text-xs border border-gray-300 rounded focus:ring-1 focus:ring-indigo-500 outline-none font-mono" placeholder="booking-main (自動生成可)" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Channel ID</label>
                  <input value={lineAccountForm.channelId} onChange={e => setLineAccountForm({ ...lineAccountForm, channelId: e.target.value })}
                    className="w-full px-2 py-1.5 text-xs border border-gray-300 rounded focus:ring-1 focus:ring-indigo-500 outline-none font-mono" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Channel Secret *</label>
                  <input value={lineAccountForm.channelSecret} onChange={e => setLineAccountForm({ ...lineAccountForm, channelSecret: e.target.value })}
                    className="w-full px-2 py-1.5 text-xs border border-gray-300 rounded focus:ring-1 focus:ring-indigo-500 outline-none font-mono" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Channel Access Token *</label>
                  <input value={lineAccountForm.channelAccessToken} onChange={e => setLineAccountForm({ ...lineAccountForm, channelAccessToken: e.target.value })}
                    className="w-full px-2 py-1.5 text-xs border border-gray-300 rounded focus:ring-1 focus:ring-indigo-500 outline-none font-mono" />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-medium text-gray-700 mb-1">Basic ID</label>
                    <input value={lineAccountForm.basicId} onChange={e => setLineAccountForm({ ...lineAccountForm, basicId: e.target.value })}
                      className="w-full px-2 py-1.5 text-xs border border-gray-300 rounded focus:ring-1 focus:ring-indigo-500 outline-none" placeholder="@xxx" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-gray-700 mb-1">招待URL</label>
                    <input value={lineAccountForm.inviteUrl} onChange={e => setLineAccountForm({ ...lineAccountForm, inviteUrl: e.target.value })}
                      className="w-full px-2 py-1.5 text-xs border border-gray-300 rounded focus:ring-1 focus:ring-indigo-500 outline-none" />
                  </div>
                </div>
              </div>
              <div className="flex gap-2 mt-5">
                <button onClick={handleLineAccountSave} disabled={lineAccountSaving}
                  className="flex-1 px-3 py-2 bg-indigo-600 text-white text-xs rounded-lg hover:bg-indigo-700 disabled:opacity-50">
                  {lineAccountSaving ? '保存中...' : lineAccountEditing ? '更新' : '追加'}
                </button>
                <button onClick={() => setLineAccountModalOpen(false)}

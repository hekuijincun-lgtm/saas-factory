...
export function registerOwnerMarketingRoutes(app: Hono<{ Bindings: Record<string, unknown> }>) {
  // ── Debug: account & token diagnostic ─────────────────────────

  app.get("/owner/marketing/debug/post/:vertical", async (c) => {
    ...
    if (!account) {
      // List all instagram-related keys for diagnosis
      const allKeys = await kv.list({ prefix: "owner:instagram:" });
      const keyNames = (allKeys.keys ?? []).map((k: any) => k.name);
      return c.json({ ok: false, error: "account_not_found", triedKey: `owner:instagram:account:${vertical}`, existingKeys: keyNames });
    }
    ...
    const info: any = {
      ...
      tokenPrefix: account.accessToken?.slice(0, 20) + "...",
      tokenExpiresAt: account.tokenExpiresAt,
      tokenExpired: account.tokenExpiresAt < Date.now(),
      autoPost: account.autoPost,
    };

    // Verify token with Meta debug_token if META_APP_ID/SECRET available
    const appId: string | undefined = env.META_APP_ID;
    const appSecret: string | undefined = env.META_APP_SECRET;
    if (appId && appSecret) {
      try {
        const verifyRes = await fetch(
          `https://graph.facebook.com/debug_token?input_token=${encodeURIComponent(account.accessToken)}&access_token=${appId}|${appSecret}`
        );
        info.tokenVerify = await verifyRes.json();
      } catch (e: any) {
        info.tokenVerifyError = e?.message;
      }
    } else {
      info.tokenVerifySkipped = "META_APP_ID or META_APP_SECRET not set";
    }

    // Try a simple Graph API call to verify token works
    try {
      const meRes = await fetch(`https://graph.facebook.com/v19.0/${account.igUserId}?fields=id,username&access_token=${encodeURIComponent(account.accessToken)}`);
      info.graphApiTest = { status: meRes.status, body: await meRes.json() };
    } catch (e: any) {
      info.graphApiTestError = e?.message;
    }
  ...
  app.post("/owner/marketing/post/:vertical", async (c) => {
    ...
    const account = await kvGetJson<IgAccount>(kv, `owner:instagram:account:${vertical}`);
    if (!account) {
      return c.json({ ok: false, error: "IGアカウント未登録", detail: `${vertical} のInstagramアカウントを先に /owner/marketing/accounts で登録してください` }, 404);
    }
    if (!account.accessToken || !account.igUserId) {
      return c.json({ ok: false, error: "IGアカウント設定不完全", detail: "igUserId または accessToken が未設定です" }, 400);
    }

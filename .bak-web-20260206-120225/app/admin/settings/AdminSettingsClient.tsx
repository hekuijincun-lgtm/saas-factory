...
export default function AdminSettingsClient() {
  ...
  const fetchLineStatusState = async () => {
    try {
      ...
      // prefer existing fetchLineStatusState() if present
      await fetchLineStatusState(); return;// fallback: call proxy directly (tenantId defaults to "default")
      const tid =
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <span className="text-xs px-2 py-1 rounded-full border bg-gray-50">
            {loadingStatus || !lineStatus
              ? "確認中..."
              : (((lineStatus as any).kind === "linked") || ((lineStatus as any).kind === "connected"))
              ? "連携済み"
